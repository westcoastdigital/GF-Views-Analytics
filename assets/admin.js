...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    // Only default to last 30 days if no URL params present
  ...
  function initDatePickers() {
    ...
    flatpickr("#gfva-date-from", {
      ...opts,
      onChange: (d, s) => {
        state.dateFrom = s;
      },
    });
    flatpickr("#gfva-date-to", {
      ...opts,
      onChange: (d, s) => {
        state.dateTo = s;
      },
    });
    flatpickr("#gfva-compare-from", {
      ...opts,
      onChange: (d, s) => {
        state.compareFrom = s;
      },
    });
    flatpickr("#gfva-compare-to", {
      ...opts,
      onChange: (d, s) => {
        state.compareTo = s;
      },
  ...
  function initFormSelect() {
    ...
    $("#gfva-forms-search").on("input", function () {
      ...
      $(
        "#gfva-forms-options .gfva-multiselect__option:not(.gfva-multiselect__option--all)",
      ).each(function () {
  ...
  function loadForms() {
    $.post(
      GFVA.ajax_url,
      { action: "gfva_get_forms", nonce: GFVA.nonce },
      function (res) {
        if (!res.success) return;
        state.forms = res.data;
        renderFormOptions(res.data);

        // Restore selected forms from URL after options are rendered
  ...
  function renderFormOptions(forms) {
    ...
    $opts.append(
      $(
        '<label class="gfva-multiselect__option gfva-multiselect__option--all">',
      ).append(
    ...
    $opts.on("change", ".gfva-form-cb", function () {
      ...
      state.selectedForms = $(".gfva-form-cb:checked")
        .map(function () {
  ...
  function runReport() {
    ...
    history.pushState(
      null,
      "",
      "?" + params.toString() + "&page=gf-views-analytics",
    );
  ...
  function renderStats(data) {
    ...
    $("#stat-unique-views").text(formatNumber(p.unique_views));
    ...
    if (c) {
      ...
      renderDelta("#stat-unique-views-cmp", p.unique_views, c.unique_views);
      if (state.includeEntries) {
        renderDelta(
          "#stat-total-entries-cmp",
          p.total_entries,
          c.total_entries,
        );
  ...
  function renderDelta(selector, current, previous, isPct) {
    if (!previous && !current) {
      $(selector).text("").removeClass("up down flat");
      return;
    }
  ...
  function renderMainChart(data) {
    ...
    mainChart = new Chart(canvas, {
      ...
      options: {
        ...
        scales: {
          x: {
            grid: { color: "#f0f2f5" },
            ticks: { color: "#718096", font: { size: 11 }, maxTicksLimit: 12 },
          },
          y: {
            grid: { color: "#f0f2f5" },
    ...
    $("#gfva-chart-title").text(
      compare ? "Views over time (with comparison)" : "Views over time",
    );
  }
  ...
  function renderTable(data) {
    ...
    const cols = ["Period", "Views"];
    if (compare) cols.push("Views (compare)", "Views Δ");
    if (state.includeEntries && primary.entries) {
      cols.push("Entries");
      if (compare && compare.entries)
        cols.push("Entries (compare)", "Entries Δ");
      cols.push("Conv. %");
    }

    const $thead = $("#gfva-table-head").empty();
    const $hrow = $("<tr>");
    cols.forEach((c) => $hrow.append(`<th>${escHtml(c)}</th>`));
    $thead.append($hrow);
    ...
    allPeriods.forEach(function (period) {
      ...
      const cEntries =
        compare && compare.entries ? compare.entries[period] || 0 : null;
      const conv =
        views > 0 && entries !== null
          ? ((entries / views) * 100).toFixed(1)
          : null;
      const vDelta = cViews !== null ? views - cViews : null;
      const eDelta =
        cEntries !== null && entries !== null ? entries - cEntries : null;

      const $tr = $("<tr>");
      $tr.append(`<td>${escHtml(period)}</td>`);
      $tr.append(`<td class="num">${formatNumber(views)}</td>`);
      if (compare !== null) {
        $tr.append(`<td class="num">${formatNumber(cViews)}</td>`);
        $tr.append(deltaCell(vDelta));
      }
      if (state.includeEntries && entries !== null) {
        $tr.append(`<td class="num">${formatNumber(entries)}</td>`);
        if (cEntries !== null) {
          $tr.append(`<td class="num">${formatNumber(cEntries)}</td>`);
          $tr.append(deltaCell(eDelta));
        }
        $tr.append(`<td class="num">${conv !== null ? conv + "%" : "—"}</td>`);
      }
      $tbody.append($tr);
    });
  }
  ...
  function deltaCell(delta) {
    ...
    return `<td class="num ${cls}">${sign}${formatNumber(delta)}</td>`;
  }
  ...
  function renderPrintHeader() {
    ...
    const forms = state.selectedForms.length
      ? state.forms
          .filter((f) => state.selectedForms.includes(f.id))
          .map((f) => f.title)
          .join(", ")
      : "All forms";
  ...
  function exportCsv() {
    ...
    if (state.includeEntries && primary.entries) {
      cols.push("Entries");
      if (compare && compare.entries)
        cols.push("Entries_Compare", "Entries_Delta");
    ...
    allPeriods.forEach(function (period) {
      ...
      const row = [period, views];
      if (cViews !== null) {
        row.push(cViews, views - cViews);
      }
      if (entries !== null) {
        row.push(entries);
  ...
  function readUrlParams() {
    ...
    if (params.get("granularity")) {
      ...
      $("#gfva-granularity .gfva-seg-btn").removeClass("active");
      $(
        `#gfva-granularity .gfva-seg-btn[data-value="${state.granularity}"]`,
      ).addClass("active");
    }
    ...
    // If URL has date params, auto-run
    if (params.get("date_from") && params.get("date_to")) {
      // Forms dropdown may not be rendered yet — wait for loadForms to finish
      state._autoRun = true;
    }
  }
  ...
  function mergeKeys(objects) {
    const set = new Set();
    objects.forEach(
      (obj) => obj && Object.keys(obj).forEach((k) => set.add(k)),
    );
